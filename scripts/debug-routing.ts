...
const testCases = [
    ...
    { url: "http://localhost:3000/ysws/", host: "localhost:3000", expected: true },
    { url: "http://localhost:3000/admin/ysws", host: "localhost:3000", expected: true },
    { url: "http://localhost:3000/my-bucket", host: "localhost:3000", expected: false },
    { url: "http://ysws.localhost:3000/", host: "ysws.localhost:3000", expected: true }, // Should be true if wildcard localhost
    { url: "http://other.localhost:3000/", host: "other.localhost:3000", expected: true }, // Should be true if wildcard localhost??
...
testCases.forEach(tc => {
    ...
    console.log(`URL: ${tc.url} | Host: ${tc.host} | Expected: ${tc.expected} | Got: ${result} | ${result === tc.expected ? "PASS" : "FAIL"}`);
});
