	...
	try {
		...
		try {
			...
			if (error.$metadata?.httpStatusCode === 501) {
				...
				console.log(
					`⚠️ Get Bucket Policy failed with status: ${error.$metadata?.httpStatusCode}`,
				);
			}
		}
		...
		try {
			...
			if (res.status === 501) {
				...
				console.error(`❌ Put Bucket ACL allowed with status: ${res.status}`);
			}
		} catch (_e) {
		...
		try {
			...
			if (
				...
				console.log(
					`⚠️ Cross-Tenant Access failed with unexpected status: ${error.$metadata?.httpStatusCode}`,
				);
			}
		}
		...
		try {
			...
			if (error.$metadata?.httpStatusCode === 403) {
				...
				console.log(
					`⚠️ Cross-Tenant Copy failed with unexpected status: ${error.$metadata?.httpStatusCode}`,
				);
			}
		}
