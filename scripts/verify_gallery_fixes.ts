...
describe("YSWS Gallery Fixes Verification", () => {
    let testUserId: string;

    beforeAll(async () => {
        // Create a test user
        const [user] = await db.insert(users).values({
            id: `verify_user_${Date.now()}`,
    ...
    it("should fetch approved submissions for gallery", async () => {
        ...
        const approved = publicSubmissions.find(s => s.projectName === "Verify Approved");
