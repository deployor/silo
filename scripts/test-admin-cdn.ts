...
async function runAdminCdnTest() {
	...
	try {
		...
		const internalPath = getInternalPath("test.txt", { id: userId } as any, bucket[0]);
		...
		const deleteRes = await fetch(`${endpoint}/api/admin/buckets/${bucketName}`, {
			method: "DELETE",
			headers: {
		...
		const resetRes = await fetch(`${endpoint}/api/admin/buckets/${bucketName}?reset=true`, {
			method: "DELETE",
			headers: {
				Cookie: `silo_user_id=${adminId}`,
			},
