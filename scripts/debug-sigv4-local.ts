...
function canonicalize(req: Request) {
  ...
  if (!authHeader.startsWith("AWS4-HMAC-SHA256")) throw new Error("No SigV4 Authorization header");
  ...
  const amzDate = headers.get("X-Amz-Date") ?? headers.get("x-amz-date") ?? headers.get("Date") ?? "";
  ...
  if (headers.has("x-amz-content-sha256")) hashedPayload = headers.get("x-amz-content-sha256") || "";
  else if (headers.has("X-Amz-Content-Sha256")) hashedPayload = headers.get("X-Amz-Content-Sha256") || "";
...
async function main() {
  ...
  if (!accessKeyId || !secretAccessKey) throw new Error("Need SIGV4_ACCESS_KEY and SIGV4_SECRET_KEY");
  ...
  console.log("x-amz-content-sha256:", signed.headers.get("x-amz-content-sha256"));

  const req = new Request(signed.url, { method: "PUT", headers: signed.headers, body: "hello" });

  const { canonicalRequest, credential, signature, amzDate } = canonicalize(req);
  ...
  const calc = createHmac("sha256", kSigning).update(stringToSign).digest("hex");
