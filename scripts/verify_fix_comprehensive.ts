...
async function run() {
    ...
    const testFiles = [
        ...
        { name: "test-medium.bin", size: 1024 * 1024 * 5, data: new Uint8Array(1024 * 1024 * 5).fill(66) }, // 5MB
        { name: "test-large.bin", size: 1024 * 1024 * 15, data: new Uint8Array(1024 * 1024 * 15).fill(67) }, // 15MB
    ...
    for (const file of testFiles) {
        ...
        if (cl === file.size.toString()) {
            ...
            console.error(`✗ ${file.name}: GET Content-Length MISMATCH (Expected ${file.size}, got ${cl})`);
            getFailures++;
        }
    ...
    for (const file of testFiles) {
        ...
        // A. List Object
        const listRes = await s3Client.fetch(`?list-type=2&prefix=${file.name}`, { method: "GET" });
        ...
        if (resolvedSize === file.size.toString()) {
            ...
            console.error(`  ✗ Resolved Size WRONG: ${resolvedSize} (Expected ${file.size})`);
            migrationFailures++;
        }
    }
