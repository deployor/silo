...
function env(): Env {
  ...
  if (!Number.isFinite(perfConcurrency) || perfConcurrency <= 0) throw new Error("PERF_CONCURRENCY invalid");
  if (!Number.isFinite(perfIterations) || perfIterations <= 0) throw new Error("PERF_ITERATIONS invalid");
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) throw new Error("TIMEOUT_MS invalid");
...
async function perfPutGet(
  ...
  async function worker(workerId: number) {
    while (true) {
      ...
      if (doPut) {
        ...
        if (cfg.verbose && !ok) {
          console.error(`[PUT FAIL] status=${res.status} body=${await res.text().catch(() => "")}`);
        }
        addLat(putLat, ms, ok, res.status);
        putSamples.push(ms);
      } else {
        // presigned GET (query)
        const s = await aws.sign(putUrl, { method: "GET", aws: { signQuery: true } });
        const t0 = performance.now();
        const res = await fetchWithTimeout(s.url, { method: "GET" }, cfg.timeoutMs);
        const ms = performance.now() - t0;
        const ok = res.status >= 200 && res.status < 300;
        if (cfg.verbose && !ok) {
          console.error(`[GET FAIL] status=${res.status} body=${await res.text().catch(() => "")}`);
        }
  ...
  const t0 = performance.now();
  await Promise.all(Array.from({ length: cfg.perfConcurrency }, (_, w) => worker(w)));
...
async function perfList(aws: AwsClient, cfg: Env, runPrefix: string) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const t0 = performance.now();
    const res = await fetchWithTimeout(s.url, { method: "GET", headers: s.headers }, cfg.timeoutMs);
    const ms = performance.now() - t0;
    const ok = res.status >= 200 && res.status < 300;
...
async function securityProbes(aws: AwsClient, cfg: Env, runPrefix: string) {
  ...
  //    We expect deny (400/401/403/404/429) but NOT 2xx.
  {
    ...
    if (!putRes.ok) throw new Error(`seed dup-query PUT failed status=${putRes.status}`);

    const signedGet = await aws.sign(putUrl, { method: "GET", aws: { signQuery: true } });
  ...
  // 3) Header smuggling-ish: duplicate headers are hard in fetch, but we can try weird whitespace.
  {
    ...
    const res = await fetchWithTimeout(putUrl, { method: "PUT", headers: h, body: "ws" }, cfg.timeoutMs);
...
async function main() {
  ...
  for (const size of cfg.perfSizes) {
    ...
    console.log("\nPUT+GET size:", size);
    console.log(`ops: ${r.totalOps}  wall: ${(r.wallMs / 1000).toFixed(2)}s  ops/s: ${r.opsPerSec.toFixed(2)}`);
    console.log(`PUT ok/fail: ${r.putLat.ok}/${r.putLat.fail}  p50=${r.putP50.toFixed(1)}ms  p95=${r.putP95.toFixed(1)}ms`);
    console.log(`GET ok/fail: ${r.getLat.ok}/${r.getLat.fail}  p50=${r.getP50.toFixed(1)}ms  p95=${r.getP95.toFixed(1)}ms`);
  }

  const list = await perfList(aws, cfg, runPrefix);
  console.log("\nLIST p50/p95:", list.p50.toFixed(1), "/", list.p95.toFixed(1), "ms", "prefix:", list.prefix);

  console.log("\n== Security probes ==");
  const sec = await securityProbes(aws, cfg, runPrefix);
  const failed = sec.filter((x) => !x.ok);
  for (const r of sec) {
    console.log(`${r.ok ? "OK" : "FAIL"}  ${r.name}  status=${r.status ?? "?"}  ${r.note}`);
  }
