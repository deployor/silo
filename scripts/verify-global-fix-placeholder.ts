...
async function _verifyGlobalFix() {
	...
	const _endpoint = config.s3.endpoint
		.replace(/^https?:\/\//, "")
		.replace(/\/$/, "");
...
console.log(
	"Skipping live server test in this script, relying on code inspection and previous tests.",
);
