...
function getEnv(): Env {
	...
	if (!Number.isFinite(expires) || expires <= 0)
		throw new Error("SIGV4_EXPIRES invalid");
	if (!Number.isFinite(iterations) || iterations <= 0)
		throw new Error("SIGV4_ITERATIONS invalid");
	if (!Number.isFinite(concurrency) || concurrency <= 0)
		throw new Error("SIGV4_CONCURRENCY invalid");
	if (!Number.isFinite(timeoutMs) || timeoutMs <= 0)
		throw new Error("SIGV4_TIMEOUT_MS invalid");
...
function parseQuery(u: URL): Array<[string, string]> {
	...
	return raw.split("&").map((kv) => {
		...
		return [
			decodeURIComponent(kv.slice(0, idx)),
			decodeURIComponent(kv.slice(idx + 1)),
		];
	});
}
...
function mutatePresignedUrl(original: string): MutationResult[] {
	...
	// Duplicate a signed parameter (should deny).
	{
		...
		const candidates = p.filter(
			([k]) => k.startsWith("X-Amz-") && k !== "X-Amz-Signature",
		);
		if (candidates.length) {
			...
			mutations.push({
				...
				expect: "deny",
				reason:
					"Duplicate params can change canonical query; should be rejected",
			});
		}
	}
...
function statusBucket(stats: Stats, status: number) {
	stats.unexpectedStatus.set(
		status,
		(stats.unexpectedStatus.get(status) ?? 0) + 1,
	);
}
...
async function main() {
		...
		while (true) {
			...
			for (const t of tests) {
				try {
					...
					if (env.verbose) {
						console.error(
							`[FETCH-ERROR] worker=${workerId} iter=${my} test=${t.name} ${msg}`,
						);
					}
					await sleep(25);
				}
	...
	console.log(
		`iterations: ${env.iterations}  (cases/iter: ${totalCasesPerIter})`,
	);
	...
	console.log(
		`  fail: ${stats.allowFail} (${pct(stats.allowFail, allowTotal)})`,
	);
	...
	if (stats.unexpectedStatus.size) {
		const sorted = [...stats.unexpectedStatus.entries()].sort(
			(a, b) => b[1] - a[1],
		);
