...
export async function handleAdminYswsRequest(
	req: Request,
	...
	if (match) {
		...
		if (req.method === "GET") {
			return new Response(
				await render("admin-ysws-review", {
		...
		if (req.method === "POST") {
			...
			if (action === "approve") {
				await YswsService.approveSubmission(
					id,
					user.id,
					publicNotes,
					privateNotes,
				...
				await YswsService.rejectSubmission(
					id,
					user.id,
					publicNotes,
					privateNotes,
