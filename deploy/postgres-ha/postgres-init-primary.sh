...
: "${POSTGRES_HA_CONTROLLER_USER:?POSTGRES_HA_CONTROLLER_USER is required}"
: "${POSTGRES_HA_CONTROLLER_PASSWORD:?POSTGRES_HA_CONTROLLER_PASSWORD is required}"
...
psql --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -v ON_ERROR_STOP=1 \
  ...
  --set=controller_user="$POSTGRES_HA_CONTROLLER_USER" \
  --set=controller_password="$POSTGRES_HA_CONTROLLER_PASSWORD" <<'SQL'
...
SELECT format('CREATE ROLE %I LOGIN SUPERUSER PASSWORD %L', :'controller_user', :'controller_password')
WHERE NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = :'controller_user') \gexec
...
hostssl all             ${POSTGRES_HA_CONTROLLER_USER} 0.0.0.0/0 scram-sha-256
...
if [ -n "${POSTGRES_MIGRATION_PUBLIC_IP:-}" ]; then
  printf 'hostssl all all %s/32 scram-sha-256\n' "$POSTGRES_MIGRATION_PUBLIC_IP" >> "$PGDATA/pg_hba.conf"
fi
